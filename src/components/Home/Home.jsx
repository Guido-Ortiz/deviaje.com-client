...
export default function Home() {
  ...
  const currentFlights =
    flights && flights?.slice(indexOfFirstFlight, indexOfLastFlight);
  ...
  return (
    <div className={styles.containerGeneral}>
      {showDetails && (
        <ModalDetails idDetails={idDetails} setShowDetails={setShowDetails} />
      )}
      {showDetailsReturn && (
        <ModalDetailsReturn
          idDetails={idDetails}
          setShowDetails={setShowDetailsReturn}
        ...
        {dataInputs.toFrom
          ? currentFlights?.map((f) => (
              <CardFromTo
                key={f.id}
                f={f}
                handleDetails={handleDetailsReturn}
              />
            ))
          : currentFlights?.map((f) => (
